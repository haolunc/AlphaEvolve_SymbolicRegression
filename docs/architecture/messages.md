...
Produced by `ProgramsDatabase.get_prompt()`, consumed by samplers.
...
**Queue:** `prompt_queue` (distributed mode) or returned directly (single-chain mode).

---

## SampleMessage

Produced by sampler workers, consumed by evaluator workers. Bundles an LLM response with sampling metadata.
...
**Queue:** `sample_queue` (distributed mode) or created inline (single-chain mode).
...
**Queue:** `result_queue` carries `(EvalResult, SampleMessage)` tuples in distributed mode.
...
| Field | Type | Description |
|-------|------|-------------|
| `worker_type` | `str` | `"sampler"`, `"evaluator"`, or `"database"` |
| `worker_id` | `int` | Worker index |
| `stats` | `dict` | Arbitrary key-value performance metrics |
