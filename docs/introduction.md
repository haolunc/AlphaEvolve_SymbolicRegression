...
flowchart LR
    ...
    P -->|prompt| LLM["LLM"]
    LLM -->|candidate code| E["Evaluator"]
    E -->|score +<br/>complexity| DB
```
...
3. The LLM's candidate is executed and scored by the **evaluator**.
4. The result (score, complexity) is registered back into the database, and the loop repeats.
...
| Source | Content |
|--------|---------|
| **User-provided** | Task description (from the specification docstring) |
...
Programs are sorted by score in **ascending** order so the LLM sees a worst-to-best progression, priming it to improve on the best.
...
- **Complexity** = weighted AST node count.
...
1. Rank islands by best score.
2. Reset the bottom 50%.
